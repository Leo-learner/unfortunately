...
MCP 支持通过 `UNFORTUNATELY_ADMIN_KEY_FILE` 指定本机密钥文件（默认 `~/.config/unfortunately-mcp/admin-key.txt`）。密钥格式为 `uf_ai_` 加 32 字节随机数的十六进制表示，文件权限须为 600。工具每次请求自动读取文件并发送独立 Bearer 凭证，无需邮箱验证码、无需反复登录，不会把密钥放入工具结果或客户端配置文本。

网站通过 `MCP_ADMIN_KEY_HASH_FILE` 读取该密钥的 SHA-256 摘要；服务器不保存原始密钥。此密钥拥有站长权限，独立于原有邮箱和访客会话。密钥模式优先于邮箱会话，不设置自动过期。更换或撤销需更新/移除服务器摘要并重启服务；邮箱退出不会撤销密钥。公开投递和统计请求不携带密钥，网络请求禁止跟随重定向。

部署配置工具 `deploy/configure-mcp-key.mjs` 从标准输入读取摘要，先备份已有配置，再保存私有摘要文件与环境变量。须按项目部署顺序在已验证、已推送的发布目录中执行。原始密钥只能保存在仓库外本机私有文件中。
