...
`mcp/` 提供独立本机 stdio MCP，包含投递增删改查、导出、统计和评论回复等 14 项操作。通过本机终端进行邮箱验证，AI 工具不处理验证码或暴露登录文件。站长、访客权限由现有线上 API 校验。安装、登录和接入说明见 [MCP README](mcp/README.md)。该模块不增加远程 MCP URL，也不改变网站的部署和数据库。
